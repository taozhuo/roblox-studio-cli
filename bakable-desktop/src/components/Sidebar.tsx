...
export default function Sidebar({
  ...
  // Poll selection when sidebar is open
  useEffect(() => {
    if (!open) return;

    const fetchSelection = async () => {
      try {
        const res = await fetch('http://127.0.0.1:4849/devtools/call', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ tool: 'studio.selection.get', params: {} })
        });
        const data = await res.json();
        if (data.success && data.result?.items) {
          setSelection(data.result.items);
        }
      } catch (e) {
    ...
    fetchSelection();
    const interval = setInterval(fetchSelection, 1000);
      ...
      <aside className={`sidebar ${open ? 'open' : ''}`}>
        ...
        <header className="sidebar-header">
          <div className="sidebar-brand">
            ...
            <div>
              <h1 className="sidebar-title">Bakable</h1>
              ...
              {selection.length === 0 ? (
                ...
                selection.map((item, i) => (
                  <div key={i} className="selection-item">
                    <span className="selection-class">{item.className}</span>
        ...
        <footer className="sidebar-footer">
          <a href="https://bakable.ai" target="_blank" rel="noopener noreferrer" className="footer-link">
          ...
          <a href="https://github.com/bakable" target="_blank" rel="noopener noreferrer" className="footer-link">
